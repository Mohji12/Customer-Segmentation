...
from workflow.Transformation import DateTransformer
from workflow.visualization import generate_cluster_plots

st.set_page_config(layout="wide")

# Load the pre-trained KMeans pipeline
with open('Model Building/GradientBoosting_Pipline.pkl', 'rb') as f:
    loaded_pipeline = pickle.load(f)

df = pd.read_csv('Model Building/Segmented_Data',index_col=0)
...
if submitted:
    ...
    st.write(f'<span style="font-family: Arial; font-size: 24px; font-weight: bold; color: #fcba03;">Predicted Cluster: {cluster}</span>', unsafe_allow_html=True)
    st.write(f'<span style="font-family: Arial; font-size: 18px; color: #fcba03;">{cluster_description[cluster]}</span>', unsafe_allow_html=True)
